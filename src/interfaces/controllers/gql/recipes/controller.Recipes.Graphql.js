...
export const getVideoRecipes = async (page = 1, limit = 50) => {
  try {
    const recipes = await Recipe.find({
      video: { $ne: null },
    })
    ...
    logger.error({ message: "Failed to get video recipes" });
    throw new Error("Failed to get video recipes");
  }
};

export const getAutocompleteSuggestions = async (userInput) => {
  const keywords = userInput.split(' ').map(keyword => `(${keyword})`).join('|');
  ...
  try {
    ...
    logger.error({ message: "Failed to get autocomplete suggestions" });
